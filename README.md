...
- 🧩 **Плиты без дыр:** текстура плиты больше не «полупрозрачная» и не рвётся — боковые грани берут свою половину тайла, верхняя плита использует верхнюю половину, а сетка трещин при ломании повторяет форму плиты (полблока), а не полного куба
...
- 🚧 **Забор:** столб 3/8×1½ блока, сам соединяется перекладиной с соседним забором; через одиночный столб можно перепрыгнуть, вплотную к нему не пройти
...
| **Верстак** (поставить блок и нажать ПКМ) — сетка 3×3 | деревянные и каменные инструменты, **лук**, печь, **сундук** (8 досок кольцом), **забор** (доски + палки), **наковальня** (4 слитка железа + 2 угля + 2 доски), хлеб, кирпич, камень, сланец, стекло |
