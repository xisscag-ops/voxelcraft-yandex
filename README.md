...
- 🎮 **Два режима:** «Выживание» и «Креатив» — переключаются в главном меню и на паузе, выбор хранится в сохранении
...
- ⚒ **Крафт из 23 рецептов:** сетка 2×2 в инвентаре + блок **верстак** для рецептов 3×3 (кирки, топоры, мечи, лук); доски, палки, стрелы, стекло, кирпич, камень, сланец, светокамень, снег, полублоки, факелы и печка
- 🔥 **Печка:** ПКМ по поставленной печке открывает обжиг железной/золотой руды и выпечку хлеба из пшеницы; топливо — уголь или доски. Руда падает в мир предметом и подбирается при приближении
- 🔦 **Факелы** освещают окружающие блоки ночью; половинки деревянных и каменных блоков можно ставить сверху/снизу, автоматически зашагивать на них и складывать в целый блок
- 🙌 **Предмет в руке:** блок — объёмный кубик с настоящими текстурами атласа, инструменты, палка и яблоко — детальные 3D-модели, лук — дуга с тетивой, которая натягивается при выстреле
...
- 🌍 Процедурный бесконечный мир: холмы, горы с каменными хребтами и снежными вершинами, пляжи, вода, деревья
- 🕳 **Пещеры:** извилистые тоннели, крупные подземные залы, лазы с поверхности (в каждом чанке раскрыт вход к ближайшей полости), сталактиты и сталагмиты, гравий, мох на стенах и руды в камне
...
- 🐇 Крупные мобы с лицами: у зайчиков глаза и зубки, у барашков рога и оскал, у слизней широкий рот с зубами; ночной охотник **Хмарь** — небольшой балахон с капюшоном, пугающим лицом, четырьмя светящимися глазами, когтистыми лапами, шипами и рычанием
- 🕷 **Чёрные пауки** выходят ночью и кусают, **криперы** подкрадываются и взрываются (с шипением и разрушением блоков), в воде плавает **рыба**, по земле рыщут **волки**: раненый волк впадает в ярость и гонится за обидчиком
- 👀 Мобы появляются реже и только позади игрока, вне поля зрения; убитые игроком оставляют подбираемые сферы опыта для повышения уровня
- 💀 **Анимация смерти:** убитый моб не исчезает мгновенно — заваливается набок, оседает, уменьшается и рассыпается частицами
- 🧱 40 видов блоков: руды (уголь, железо, золото, алмаз), гравий, песчаник, лёд, мшистый камень, обсидиан, берёза, ель, кактус; хотбар с выбором колесом/цифрами/тапом
...
- 💥 Анимация урона и бега: при беге камера мерно покачивается; при уроне — красная вспышка на весь экран, тряска камеры и интерфейса, мигание сердечек; мобы на 0.3 с становятся красными, отлетают назад (не сквозь блоки), пищат и сыплют красными частицами
...
| Где | Что доступно |
|---|---|
| Сетка **2×2** в окне инвентаря (E) | бревно → доски, палки, стрелы, светокамень, снег, 2 доски/булыжника → 4 полублока, уголь + палка → 4 факела, 4 доски → **верстак** |
| **Верстак** (поставить блок и нажать ПКМ) — сетка 3×3 | кирки, топоры и мечи из дерева и камня, **лук**, кирпич, камень, сланец, стекло, 8 булыжников → **печка** |
| **Печка** (ПКМ по поставленному блоку) | руда железа/золота → слитки, 3 пшеницы → хлеб; за каждую операцию расходуется уголь или доска |
...
Рецепты хлеба и слитков доступны и в обычном крафте (как в предыдущей версии), и в печке.
...
| | Выживание | Креатив |
|---|---|---|
| Сломанный блок | попадает в инвентарь (камень → булыжник), а руды падают предметами на землю; стекло пропадает | исчезает |
...
Режим можно переключить в главном меню до продолжения сохранённого мира и на экране паузы
во время игры. «Новый мир» также предлагает выбрать режим; он хранится в сейве. Старые сохранения без режима открываются в креативе. Сохранения обеих веток (PR #6 и main)
автоматически переводятся в новый формат: правки мира, факелы, печки и предметы получают
совместимые ID. Очень большие старые запасы ограничиваются новыми 36 слотами инвентаря.

Клавиша **F** съедает яблоко или хлеб — в режим полёта переводит двойное нажатие пробела (креатив).
...
| ПКМ — поставить (с луком — натянуть тетиву, отпустить — выстрел) | Кнопка «🧱» — поставить/выстрелить |
...
| F — съесть яблоко или хлеб | Кнопка «Еда» |
...
src/
  ...
  save-migration.js # перенос старых сохранений двух веток после слияния
...
- 🏹 В начале выживания выдаются лук, 16 стрел, 4 факела, печка и 2 буханки хлеба — стрелы подбираются с земли и с воткнувшихся в блоки
...
- 🎒 Сломанные блоки складываются в инвентарь и тратятся при установке; железо, золото, уголь и алмазы выпадают в мир предметами
- 🌾 Из высокой травы выпадает пшеница: испеките 3 единицы в печке (уголь или доска — топливо) и съешьте хлеб (+3 сердца)
- ✨ Убитые игроком мобы оставляют опыт; уровень и предметы на земле сохраняются вместе с миром
